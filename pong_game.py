...
def paddle_a_up():
    y = paddle_a.ycor()
    if y + 50 < 300:  # Check if paddle will go beyond the top boundary
        y += 20
    paddle_a.sety(y)

def paddle_a_down():
    y = paddle_a.ycor()
    if y - 50 > -300:  # Check if paddle will go beyond the bottom boundary
        y -= 20
    paddle_a.sety(y)
